...
- **UserInfo + profile enrichment** — Fetches `email` from the UserInfo endpoint and reads `hardware_verified`, `acr` and `amr` from the access token after verifying it against JWKS with `node:crypto` (no dependencies)
